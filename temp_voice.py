...
def translate_text(text: str) -> str:
    ...
    languages = ["ru", "tr", "sv", "de", "es", "ja","hi","te","ko"]
    list_translations = []
...
def text_to_speech(text: str) -> str:
    ...
    response = client.text_to_speech.convert(
        voice_id="copy the your voice id", #clone your voice on elevenlabs dashboard and copy the id
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Group() as Hindi:
            # gr.Image("flags/Hindi.png", width = 150, show_download_button=False, show_label=False)
            hi_output = gr.Audio(label="Hindi", interactive=False)
            hi_text = gr.Markdown()

        with gr.Group() as Korean:
            # gr.Image("flags/Korean.png", width = 150, show_download_button=False, show_label=False)
            ko_output = gr.Audio(label="Korean", interactive=False)
            ko_text = gr.Markdown()
        
        with gr.Group() as telugu:
            # gr.Image("flags/telugu.png", width = 150, show_download_button=False, show_label=False)
    ...
    output_components = [ru_output, tr_output, sv_output, de_output, es_output, jp_output,hi_output,te_output,ko_output,ko_text ,ru_text, tr_text, sv_text, de_text, es_text, jp_text,hi_text,te_text]
